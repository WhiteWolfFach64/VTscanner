...
def hashScanner():
    ...
    hashes = {'MD5': [], 'SHA1': [], 'SHA256': []}  # Use lists to maintain the order of appearance
    malicious_hashes = set()  # Use a set for unique malicious hashes across all files
    ...
    for line in lines:
        ...
        # Use list.extend() to add hashes to the lists
        hashes['MD5'].extend(md5_matches)
        hashes['SHA1'].extend(sha1_matches)
        hashes['SHA256'].extend(sha256_matches)

    # Convert lists to sets to remove duplicates and keep only unique hashes
    hashes['MD5'] = set(hashes['MD5'])
    hashes['SHA1'] = set(hashes['SHA1'])
    hashes['SHA256'] = set(hashes['SHA256'])
    ...
    while True:
        print("Would you like to upload this list of hashes to VT? (Yes/No)\n")
        hashesUpload = input(":")
            ...
            for SHA256_hash in hashes['SHA256']:
                ...
                if response.status_code == 200:  # Check if the response is successful
                    ...
                    if 'data' in response_data:  # Check if 'data' field exists in the response
                        ...
                        if maliciousFlag > 0:
                            print(AQUA + f"{SHA256_hash} : " + RESET + RED + BLACK + f"Hash associated with a malicious file!" + RESET + AQUA + f" {maliciousFlag} " + RESET + "vendors flagged this file hash as malicious. Proceed to manual scan!")
                            malicious_hashes.add(SHA256_hash)
                        else:
                ...
                if response.status_code == 200:  # Check if the response is successful
                    ...
                    if 'data' in response_data:  # Check if 'data' field exists in the response
                        ...
                        if maliciousFlag > 0:
                            ...
                            malicious_hashes.add(SHA1_hash)
                        else:
    ...
    while True:
        print("Would you like to export the malicious hashes to a list? (Yes/No)\n")
        export_hashes = input(":")

        if export_hashes.lower() in ('yes', 'y'):
            banner()
            nl()
            print("Here is the list of " + AQUA + str(len(malicious_hashes)) + RESET + " hashes flagged as malicious")
            for malicious_hash in malicious_hashes:
                print(REDON + malicious_hash + RESET)
            nl()
            break
        elif export_hashes.lower() in ('no', 'n'):
            banner()
            nl()
            print("Alright! Exiting...")
            nl()
    ...
    while True:
        ...
        if uploadToVT.lower() in ('yes', 'y'):
            ...
            if response_larger_file.status_code == 200:
                response_data_larger_file = response_larger_file.json()
                
                # Encoding for larger files and send the file to that URL
                url_upload_file = response_data_larger_file['data']
