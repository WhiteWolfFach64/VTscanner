...
def setProjectDirectory():
    ...
    try:
        ...
        # Remove the original VTscanner directory with sudo privileges
        if project_dir != target_dir and os.path.exists(project_dir):
            sudo_command = ["sudo", "rm", "-rf", project_dir]
            subprocess.run(sudo_command)
            progress_bar()
            print(AQUA + f"Original VTscanner directory removed: '{project_dir}'." + RESET)
        time.sleep(1)
