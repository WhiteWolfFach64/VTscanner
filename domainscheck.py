...
def VTScanning():
    while True:
        ...
        if Scan.lower() in ("yes", "y"):
            print("Proceeding to scan", end='')
            dots()
            banner()
            break
        elif Scan.lower() in ("no", "n"):
...
with open(foundFile, "r") as file:
    ...
    # Initialize a set to store the extracted domain names and avoid duplicates
    domains = set()

    for word in words:
        # Try to extract domain components from the word using tldextract
        result = tldextract.extract(word)

        # Check if the result contains a valid domain name
        if result.domain and result.suffix:
            # Include the full domain with subdomains
            full_domain_with_subdomains = f"{result.subdomain}.{result.domain}.{result.suffix}"
            domains.add(full_domain_with_subdomains)

            # Include the root domain without subdomains (if subdomain exists)
            if result.subdomain:
                root_domain = f"{result.domain}.{result.suffix}"
                domains.add(root_domain)

    domains = list(domains)  # Convert the set back to a list for printing
